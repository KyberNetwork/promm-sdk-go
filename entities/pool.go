...
func (p *Pool) swap(isToken0 bool, amountSpecified, sqrtPriceLimitX96 *big.Int) (
	...
	for state.specifiedAmount.Cmp(constants.Zero) != 0 && state.sqrtP.Cmp(sqrtPriceLimitX96) != 0 {
		...
		state.sqrtP, step.amountIn, step.amountOut, step.deltaL, err = utils.ComputeSwapStep(
		...
		state.specifiedAmount = new(big.Int).Sub(state.specifiedAmount, step.amountIn)
		state.returnedAmount = new(big.Int).Add(state.returnedAmount, step.amountOut)
		state.reinvestL = new(big.Int).Add(state.reinvestL, step.deltaL)

		if state.sqrtP.Cmp(step.nextSqrtP) != 0 {
			if state.sqrtP != step.startSqrtP {
